...
object SELECT : StatementFactory<Select>() {
    ...
    operator fun invoke(vararg column: BaseColumn<*>) = SelectPreBuilder(
        *column
    )
     ...
     **/
    infix fun <T> AVG(column: BaseColumn<T>) = SelectPreBuilder(
     ...
     **/
    infix fun <T> COUNT(column: BaseColumn<T>) = SelectPreBuilder(
